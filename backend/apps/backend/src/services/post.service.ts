...
import { Post, PostModel } from '@app/models/posts/post.schema'
import { Comment, CommentModel } from '@app/models/comments/comment.schema'
...
export class PostService {
    ...
    async findAll(user?: UserDocument, pagination: PaginationDto = new PaginationDto()): Promise<PaginatedResponseDto<PostResponseDto>> {
        ...
        const posts = await this.postModel.find({}).populate('author', 'username email').sort({ createdAt: -1 }).skip(skip).limit(limit).exec()
    ...
    async findOne(id: string, user?: UserDocument) {
        const post = await this.postModel.findOne({ _id: id }).populate('author', 'username email').populate('likesCount').exec()
    ...
    async delete(id: string, user: UserDocument) {
        ...
        await this.commentModel.delete({ post: id })
